            ...
            <div id="two">
                <div class="my-box" style="height: 250px">
                    ...
                    <form id="my-box" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
                        ...
                        <input type="hidden" name="old-password" value="<?php echo isset($_SESSION['password']) ? htmlspecialchars($_SESSION['password']) : ''; ?>">
                        <div class="container">
                            <div class="first">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = $conn->prepare("SELECT `password` FROM `employee-info` WHERE `id`= ? ");
    $sql->bindParam(1, $_SESSION['id'], PDO::PARAM_INT);
    $sql->execute();
    $old_password = $sql->fetch(PDO::FETCH_ASSOC);
    $old_password_hash = $old_password['password'];
    ...
    if (password_verify($current_password, $old_password_hash)) {
        if ($new_password === $confirm_password && !empty($new_password)) {
            ...
            $emp_password->bindParam(2, $_SESSION['id'], PDO::PARAM_INT);
